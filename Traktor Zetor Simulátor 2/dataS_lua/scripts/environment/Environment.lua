Environment = {}
Environment.RAINTYPE_RAIN = 0
Environment.RAINTYPE_HAIL = 1
source("dataS/scripts/environment/EnvironmentTimeEvent.lua")
source("dataS/scripts/environment/EnvironmentWeatherEvent.lua")
source("dataS/scripts/environment/EnvironmentPriceEvent.lua")
...
function Environment:new(skyI3DFilename, dayNightCycle, startHour, allowRain, autoRain)
  ...
  if dayNightCycle then
    ...
    instance.distanceFogCurve:addKeyframe({
      ...
      z = 0.17647058823529413,
      time = 360
    })
    instance.distanceFogCurve:addKeyframe({
      x = 0.3137254901960784,
      y = 0.29411764705882354,
      z = 0.23529411764705882,
      time = 420
    })
    instance.distanceFogCurve:addKeyframe({
  ...
  if instance.allowRain then
    ...
    instance.minRainInterval = 2880
    instance.maxRainInterval = 7200
  ...
  instance.timeUpdateIntervall = 60000
  instance.timeUpdateTime = 0
  instance.weatherUpdateTime = 0
  instance.weatherUpdateIntervall = 60000
  instance.isSunOn = true
  instance.needsLights = false
  addConsoleCommand("gsSetDayTime", "Sets the day time in hours", "consoleCommandSetDayTime", instance)
  return instance
end
...
function Environment:update(dt)
  ...
  if self.dayTime > 86400000 then
    ...
    if g_server ~= nil then
      ...
      g_server:broadcastEvent(EnvironmentPriceEvent:new(FruitUtil.fruitIndexToDesc))
  ...
  if self.dayNightCycle then
    ...
    if self.sunLightId ~= nil then
      ...
      if lightsSunThreshold > dr and lightsSunThreshold > dg and lightsSunThreshold > db then
        self.needsLights = true
      else
        self.needsLights = false
      end
  ...
  elseif self.water ~= nil then
    setVolumeFog("exp", 0.25, 0, g_currentMission.waterY, 0.18, 0.26, 0.35)
  end
  if g_server ~= nil then
    self.timeUpdateTime = self.timeUpdateTime + dt
    self.weatherUpdateTime = self.weatherUpdateTime + dt
    if self.timeUpdateTime > self.timeUpdateIntervall then
      g_server:broadcastEvent(EnvironmentTimeEvent:new(self.dayTime, self.currentDay))
      self.timeUpdateTime = 0
    end
    if self.weatherUpdateTime > self.weatherUpdateIntervall or self.timeUntilRainAfterNext ~= self.sendTimeUntilRainAfterNext or self.rainTime ~= self.sendRainTime or self.nextRainDuration ~= self.sendNextRainDuration or self.nextRainType ~= self.sendNextRainType or self.rainTypeAfterNext ~= self.sendRainTypeAfterNext then
      g_server:broadcastEvent(EnvironmentWeatherEvent:new(self.timeUntilNextRain, self.timeUntilRainAfterNext, self.rainTime, self.nextRainDuration, self.nextRainType, self.rainTypeAfterNext))
      self.sendTimeUntilNextRain = self.timeUntilNextRain
      self.sendTimeUntilRainAfterNext = self.timeUntilRainAfterNext
      self.sendRainTime = self.rainTime
      self.sendNextRainDuration = self.nextRainDuration
      self.sendNextRainType = self.nextRainType
      self.sendRainTypeAfterNext = self.rainTypeAfterNext
      self.weatherUpdateTime = 0
    end
  end
end
...
function Environment:setNextRain()
  ...
  self.rainTypeAfterNext = Environment.RAINTYPE_RAIN
  if math.random() > 0.7 then
    self.rainTypeAfterNext = Environment.RAINTYPE_HAIL
  end
...
function Environment:startRain(duration, rainType, timeUntilStart)
  ...
  self.nextRainType = Environment.RAINTYPE_RAIN
  if rainType ~= nil then
    self.nextRainType = rainType
  end
end
...
function Environment:getRainType()
  if self.nextRainType == Environment.RAINTYPE_HAIL then
...
function Environment:setEnvironmentTime(dayTime, currentDay)
  self.dayTime = dayTime
  self.currentDay = currentDay
end
function Environment:setEnvironmentWeather(timeUntilNextRain, timeUntilRainAfterNext, rainTime, nextRainDuration, nextRainType, rainTypeAfterNext)
...
function Environment:consoleCommandSetDayTime(dayTime)
  ...
  if dayTime ~= nil then
    self.dayTime = math.floor(dayTime * 1000 * 60 * 60)
