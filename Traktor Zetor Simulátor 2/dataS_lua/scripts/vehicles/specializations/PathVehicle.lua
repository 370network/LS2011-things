...
function PathVehicle:load(xmlFile)
  self.followSequence = PathVehicle.followSequence
  self.setupFromCrossroads = PathVehicle.setupFromCrossroads
  self.setVehicleToDeleted = PathVehicle.setVehicleToDeleted
  self.finallySetVehicleToDeleted = PathVehicle.finallySetVehicleToDeleted
  ...
  if self.trafficCollisionTriggerNear ~= nil and self.isServer then
  ...
  if self.trafficCollisionTriggerOthers ~= nil and self.isServer then
...
function PathVehicle:updateTick(dt)
...
function PathVehicle:draw()
  if self:getIsActiveForInput() and self.currentRoad then
...
function PathVehicle:updateAIMovement(dt)
  ...
  allowedToDrive = allowedToDrive and not self.waitingForJunction and self.numCollidingVehicles <= 0 and 0 >= self.numNearCollidingVehicles and not self.forceStopMovement
  if allowedToDrive or self.forceStopMovement then
    self.collisionWaitingTime = 0
  end
  if self.currentRoad ~= nil then
    self.steeringEnabled = false
    self.unableToDriveWaitingTime = self.unableToDriveWaitingTime + dt
    if math.abs(self.lastSpeed) > 1.0E-5 or self.forceStopMovement or not allowedToDrive then
      self.unableToDriveWaitingTime = 0
    end
    if self.collisionWaitingTime > self.honkWaitingTime then
    end
    if self.collisionWaitingTime > self.maxCollisionWaitingTime then
      self:setVehicleToDeleted()
      return
    end
    ...
    self:setVehicleToDeleted()
    local lx, lz = 0, 1
    AIVehicleUtil.driveInDirection(self, dt, 30, 1, 0.7, 26, false, true, lx, lz)
  end
end
function PathVehicle:followSequence(sequence, loopIndex, setPosition, startIndex)
  if startIndex == nil then
    startIndex = 1
  end
  self.currentSequence = sequence
  self.currentLoopIndex = loopIndex
  self.currentRoad = sequence[startIndex].road2
  self.currentDirection = sequence[startIndex].directionOnRoad2
  self.lastSplineTime = sequence[startIndex].timePos2
  self.currentSequenceIndex = startIndex + 1
  if self.currentSequenceIndex > table.getn(self.currentSequence) then
    if self.currentLoopIndex ~= 0 then
...
function PathVehicle:setVehicleToDeleted()
  ...
  if PathVehicle.isVehicleAllowedToChange(x, y, z, 50, 100) then
    self:finallySetVehicleToDeleted()
  end
end
function PathVehicle:finallySetVehicleToDeleted()
...
function PathVehicle:testScope(x, y, z, coeff, distanceSq)
  local clipDist = getClipDistance(self.components[1].node) * coeff
  if distanceSq < clipDist * clipDist then
    return true
  else
    return false
  end
end
function PathVehicle:onGhostRemove()
  self:setComponentsVisibility(false)
end
function PathVehicle:onGhostAdd()
  self:setComponentsVisibility(true)
end
