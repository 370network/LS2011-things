...
function Overlay:delete()
  if self.overlayId ~= nil and self.overlayId ~= 0 then
...
function Overlay:render()
  if self.visible and self.overlayId ~= nil and self.overlayId ~= 0 then
...
function GUIScreen:keyEvent(unicode, sym, modifier, isDown)
  for _, component in pairs(self.components) do
    if component.keyEvent ~= nil then
...
function GUIComponent:setIsVisible(visible)
  self.visible = visible
end
GUIButton = {}
...
function GUIControlsScreen:new(xPos, yPos, width, height, onClick, target)
  local bgOverlay = Overlay:new("bgOverlay", "dataS2/menu/bg_grey.png", 0, 0, 1, 1)
  local instance = GUIControlsScreen:superClass():new(bgOverlay, GUIControlsScreen_mt)
  instance.xPos = xPos
  instance.yPos = yPos
  instance.width = width
  instance.height = height
  local controlsBGOverlay = Overlay:new("controlsBGOverlay", "dataS2/menu/controlpanel.png")
  ...
  local buttonFilenames = {
    [GUIButton.STATE_NORMAL] = "dc_button_n",
    [GUIButton.STATE_FOCUSED] = "dc_button_f",
    [GUIButton.STATE_PRESSED] = "dc_button_p"
  }
  for state, filename in pairs(buttonFilenames) do
    overlays[state] = Overlay:new(filename, "dataS2/menu/" .. filename .. ".png")
  end
  instance.buttonDefaults = GUIButton:new(overlays, g_i18n:getText("ButtonDefaults"), instance.xPos + 0.04, instance.yPos + 0.05, buttonWidth, buttonHeight)
  ...
  function instance.buttonDefaults.onClick()
    ...
    instance:loadBindings("profileTemplate/inputBindingDefault.xml")
  end
  overlays = {}
  for state, filename in pairs(buttonFilenames) do
    overlays[state] = Overlay:new(filename, "dataS2/menu/" .. filename .. ".png")
  end
  instance.buttonSave = GUIButton:new(overlays, g_i18n:getText("ButtonSave"), instance.xPos + 0.4 * instance.width, instance.yPos + 0.05, buttonWidth, buttonHeight)
  ...
  function instance.buttonSave.onClick()
    ...
    InputBinding:load()
    instance.userMadeChanges = false
    instance:getScreenBelow():reset()
    gameMenuSystem.currentMenu = nil
    gameMenuSystem.optionsScreen:enable(false)
  ...
  instance.buttonCancel = GUIButton:new(overlays, g_i18n:getText("ButtonCancel"), instance.xPos + 0.7 * instance.width, instance.yPos + 0.05, buttonWidth, buttonHeight)
  ...
  function instance.buttonCancel.onClick()
    if instance.userMadeChanges then
      ...
      instance.yesNoDialog = g_gui:showGui("YesNoDialog")
      instance.yesNoDialog.target:disableBackground(false)
      instance.yesNoDialog.target:setText(g_i18n:getText("LoseChangesText"))
      instance.yesNoDialog.target:setCallbacks(instance.onLoseChanges, instance)
    else
      instance:getScreenBelow():reset()
      gameMenuSystem.currentMenu = nil
      gameMenuSystem.optionsScreen:enable(false)
...
function GUIControlsScreen:onLoseChanges(yes)
  self.isActive = true
  self.isVisible = true
  g_gui:showGui("")
  if yes then
    self.userMadeChanges = false
    self:getScreenBelow():reset()
    gameMenuSystem.currentMenu = nil
    gameMenuSystem.optionsScreen:enable(false)
    gameMenuSystem.controlsScreen:enable(false)
    g_gui:showGui("SettingsScreen")
...
function GUIControlsScreen:loadBindings(xmlFileName)
  self.list.listItems = {}
  local xmlFile = loadXMLFile("Bindings", xmlFileName)
  local i = 0
  while true do
    ...
    local inputDevice = Utils.getNoNil(getXMLInt(xmlFile, baseName .. "#device"), 0)
    ...
    for _, axesMapping in pairs(Input.AxesMapping) do
      if axesMapping.input == inputAxis then
        inputAxisName = axesMapping.name .. " (" .. inputDevice + 1 .. ")"
        break
      end
    end
  ...
  while true do
    ...
    if inputKey1 == nil and inputKey2 == nil and inputButton == nil and inputMouseButton == nil then
      print("Error: GUIBase no button or key specified for input event '" .. inputName .. "'")
      break
    end
    local key1 = ""
    if inputKey1 ~= "" then
    ...
    local gamepad = ""
    if inputButton ~= "" then
      gamepad = InputBinding.getButtonName(Input[inputButton], inputDevice)
    end
    local mouse = ""
...
<inputBinding version="]] .. tostring(InputBinding.version) .. "\">\n")
    axisPair = false
    tempListItem = nil
    for _, listItem in pairs(self.list.listItems) do
      if listItem.isAxis then
        if not axisPair then
          tempListItem = listItem
          axisPair = true
        else
          bindingsFile:write("    <axis name=\"" .. listItem.inputName .. "\" key1=\"" .. listItem.inputKey1 .. "\" key2=\"" .. tempListItem.inputKey1 .. "\" key3=\"" .. listItem.inputKey2 .. "\" key4=\"" .. tempListItem.inputKey2 .. "\" axis=\"" .. listItem.inputAxis .. "\" device=\"" .. listItem.inputDevice .. "\" invert=\"" .. listItem.inputInvert .. "\"/>\n")
          axisPair = false
        end
      elseif listItem.mouse == "--" then
        bindingsFile:write("    <input name=\"" .. listItem.inputName .. "\" key1=\"" .. listItem.inputKey1 .. "\" key2=\"" .. listItem.inputKey2 .. "\" button=\"" .. listItem.inputButton .. "\" device=\"" .. listItem.inputDevice .. "\" mouse=\"" .. listItem.mouse .. "\"/>\n")
      else
        bindingsFile:write("    <input name=\"" .. listItem.inputName .. "\" key1=\"" .. listItem.inputKey1 .. "\" key2=\"" .. listItem.inputKey2 .. "\" button=\"" .. listItem.inputButton .. "\" device=\"" .. listItem.inputDevice .. "\" mouse=\"" .. listItem.inputMouseButton .. "\"/>\n")
      end
    end
...
function GUIControlsScreen:update(dt)
  if self.isActive and self.waitForInput and self.list.listColumnNames[self.list.clickedColumn] == "gamepad" then
    local isAxis = self.list.listItems[self.list.clickedItem].isAxis
      ...
      if isAxis then
        for i = 1, Input.MAX_NUM_AXES do
          if math.abs(getInputAxis(i - 1, d - 1)) > 0.5 then
            for _, axesMapping in pairs(Input.AxesMapping) do
              if i - 1 == Input[axesMapping.input] then
                self.list.listItems[self.list.clickedItem].gamepad = axesMapping.name .. " (" .. d .. ")"
                self.list.listItems[self.list.clickedItem].inputAxis = axesMapping.input
                self.list.listItems[self.list.clickedItem].inputDevice = d - 1
                self.userMadeChanges = true
                self:endWaitForInput()
                break
              end
        ...
        for i = 1, Input.MAX_NUM_BUTTONS do
          ...
          if isDown then
            local newButtonName = InputBinding.getButtonName(i - 1, d - 1)
            for _, listItem in pairs(self.list.listItems) do
              if listItem.gamepad == newButtonName and listItem.action ~= self.list.listItems[self.list.clickedItem].action then
                self.controlsMessage2 = g_i18n:getText("ButtonAlreadyMapped")
                break
              end
            end
            self.list.listItems[self.list.clickedItem].gamepad = newButtonName
...
function GUIControlsScreen:render()
  if self.isVisible then
    ...
    renderText(self.xPos + 0.055, self.yPos + 0.18 * self.height - 0.003, 0.027, self.controlsMessage1)
    renderText(self.xPos + 0.055, self.yPos + 0.18 * self.height - 0.03 - 0.003, 0.027, self.controlsMessage2)
    setTextColor(1, 0.85, 0.7, 1)
    renderText(self.xPos + 0.046, self.yPos + 0.931 * self.height, 0.04, self.titleText)
    setTextColor(0.85, 1, 0.7, 1)
    ...
    renderText(self.xPos + 0.055, self.yPos + 0.18 * self.height, 0.027, self.controlsMessage1)
    renderText(self.xPos + 0.055, self.yPos + 0.18 * self.height - 0.03, 0.027, self.controlsMessage2)
    setTextBold(false)
  end
end
...
function GUIList:new(bgOverlay, xPos, yPos, width, height, onClick, target)
  ...
  local sliderOverlay = Overlay:new("list_slider_n", "dataS2/menu/list_slider_n.png")
  ...
  instance.listSelectionOverlay = Overlay:new("list_selection", "dataS2/menu/list_selection.png", instance.xPos + 0.005, 0, 0.1682, instance.textSize)
  local overlays = {}
  local buttonFilenames = {
...
function GUIListSlider:updateListPosition()
  ...
  if self.parentComponent.updateItemPositions then
    self.parentComponent:updateItemPositions()
  end
end
