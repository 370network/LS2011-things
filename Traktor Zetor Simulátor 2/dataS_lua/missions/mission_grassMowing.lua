MissionGrassMowing = {}
local MissionGrassMowing_mt = Class(MissionGrassMowing, FSBaseMission)
function MissionGrassMowing:new(baseDirectory, customMt)
  local mt = customMt
  if mt == nil then
    mt = MissionGrassMowing_mt
  end
  local self = MissionGrassMowing:superClass():new(baseDirectory, mt)
  self.state = BaseMission.STATE_INTRO
  self.playerStartX = 246
  self.playerStartY = 0.1
  self.playerStartZ = 270
  self.playerRotX = Utils.degToRad(0)
  self.playerRotY = Utils.degToRad(142)
  self.remainingGrassAmount = 8000
...
function MissionGrassMowing:draw()
  ...
  if self.isRunning then
    ...
    setTextBold(true)
    renderText(self.hudMissionBasePosX + 0.005, 0.929, 0.034, g_i18n:getText("mowingGrassMissionGoal") .. string.format(" %d", self.remainingGrassAmount))
    setTextBold(false)
